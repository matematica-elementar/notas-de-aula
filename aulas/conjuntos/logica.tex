...
\begin{remark}
Em toda esta seção, considere $P$ e $Q$ propriedades aplicáveis aos elementos de $\U$. Considere, também, $A = \set {x \tq x \text{ possui } P}$ e $B= \set {x \tq x \text{ possui } Q}$.
\end{remark}

\noindent Em Matemática, a Teoria de Conjuntos está intimamente relacionada à Lógica. Como evidência disso, existem diversas equivalências entre relações e operadpres de conjuntos e conectivos lógicos. Apresentar-se-ão quatro delas.

\begin{equivalence}[Inclusão e implicação] 
$A \subset B$ é equivalente a $P \implies Q$.
\end{equivalence}

\begin{equivalence}[Igualdade e bi-implicação] 
$A=B$ é equivalente a $P\iff Q$.
\end{equivalence}
...
\begin{equivalence}[Reunião e disjunção]
