...
O primeiro deles é a implicação, denotada no contexto da lógica por $\implicalogico$.  Ela constrói, a partir de duas proposições $P$ e $Q$, uma terceira proposição $P \implicalogico Q$ (lê-se $P$ implica $Q$). Na liguagem natural (pra nós, no português) a proposição $P\implicalogico Q$ é equivalente a proposição \entreaspas{se $P$, então $Q$}. O valor-verdade de $P \implicalogico Q$ é completamente determinado pelos valores-verdade de $P$ e $Q$. A relação exata pode ser consultada na Tabela \ref{tbl:implicacao}, denominada tabela-verdade.
...
De forma similar ao que ocorre na implicação, reserva-se o símbolo $\sselogico$ para contextos lógicos, utilizando o \entreaspas{$\sse$} apenas em cenários menos formais. Textualmente, uma afirmação do tipo $P \sse Q$ costuma se manifestar como \entreaspas{$P$ se, e somente se, $Q$}. Isso começa a evidenciar um importante fato sobre a bi-implicação: ela pode ser definida como a conjunção de duas implicações. A frase \entreaspas{$P$ se $Q$} é equivalente a \entreaspas{se $Q$, então $P$}, o que, conforme já visto, pode ser representado como $Q \implica P$. Ademais, \entreaspas{$P$ somente se $Q$} diz que $P$ só pode ser verdade se $Q$ também for, ou seja, $P \implica Q$. As frases, quando juntas, expressam a noção de bi-condicionalidade.

Outra versão textual comum do $\sse$ é a sentença \entreaspas{Para $P$, é necessário e suficiente que $Q$}. Assim como no caso do \entreaspas{se, e somente se}, essa expressão carrega duas informações consigo. A frase \entreaspas{Para $P$, é necessário que $Q$} significa que a validade de $P$ só é possível na presença da validade de $Q$, isto é, $P \implica Q$. Por outro lado, \entreaspas{Para $P$, é suficiente que $Q$} indica que basta $Q$ ser verdade para $P$ também ser, ou seja, $Q \implica P$.

Uma terceira forma de se apresentar a informação $P \sse Q$ é dizer que \entreaspas{$P$ é equivalente a $Q$}. Pode-se justificar essa construção pela tabela-verdade de $P \sselogico Q$. Conforme mencionado, $P \sselogico Q$ é verdade precisamente quando $P$ e $Q$ tem o mesmo valor-verdade. Em caso positivo, não existe uma diferença prática entre $P$ e $Q$. Assim, usa-se o termo \entreaspas{equivalência}, que também indica que, quando se tem uma dessas proposições, se tem a outra, já que elas possuem o mesmo valor-verdade.
...
\begin{remark}
    ...
    \emph{Um obstetra que também era matemático acabara de realizar um parto quando o pai perguntou: \entreaspas{É menino ou menina, doutor?}. E ele respondeu: \entreaspas{sim}.}
\end{remark}
